...
def generate_report(
    ...
    for i in range(num_pages):
        ...
        page_results.append({
            ...
            "word_retention": len(matched_words) / len(orig_words) * 100 if orig_words else 100.0,
        })
    ...
    lines.append(f"| Pipeline | Fully automated (zero manual adjustments) |")
    lines.append("")
    ...
    lines.append("|------|---------|")
    lines.append("| Pixel Match | % of page pixels identical (±30/255 tolerance) between original and regenerated |")
    lines.append("| F1 | Harmonic mean of content recall and precision (how well ink pixels align) |")
    lines.append("| Text ✓ | Extracted text is character-for-character identical (whitespace-normalized) |")
    lines.append("| Words | % of unique words from the original that appear in the regenerated version |")
    lines.append("")
    lines.append("---")
    lines.append(f"*Generated by ICD Writer pipeline*")
